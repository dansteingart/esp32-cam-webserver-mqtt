...
void setup() {
    ...
    */
    ...
    if (!mqttClient.connect(MQTTS, MQTTP)) {
    ...
    mqttClient.subscribe(CTOP);
    mqttClient.beginMessage(OTOP);
    mqttClient.print("and we're up");
...
void loop() {
    ...
    */
    ...
    if (!mqttClient.connected()) {
        if (Serial) Serial.println("MQTT connection starting");
        if (!mqttClient.connect(MQTTS, MQTTP)) {
        if (Serial) Serial.print("MQTT reconnection error ");
        if (Serial) Serial.println(mqttClient.connectError());
        }
    }


    if ( millis() - pongtime > last)
    {
        last = millis();
        ...
        String out = base64::encode(fb->buf, fb->len);
        ...
        mqttClient.beginMessage(ITOP,out.length(),false,1,false);
